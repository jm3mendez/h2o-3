...
public abstract class AstWhichFunc extends AstBuiltin<AstWhichFunc> {
    ...
    public abstract double op(Vec l); //Operation to perform in colWiseWhichVal(). For example, Vec.max() or Vec.min()

    public abstract String searchVal(); //String indicating what we are searching for across rows in rowWiseWhichVal(). For example, max or min.
                ...
                for (int i = 0; i < cs[0]._len; i++) {
                    ...
                    if (searchVal() == "max") {
                        for (int j = 0; j < numCols; j++) {
                            ...
                            if (Double.isNaN(val)) {
                                numNaColumns++;
                            } else if (val >= value && !(val == value)) { //Return the first occurrence of the val
                                value = val;
                                valueIndex = j;
                            }
                        }
                        ...
                        for (int j = 0; j < numCols; j++) {
                            ...
                            else if(val <=  value && !(val == value)) { //Return the first occurrence of the min index
                                value = val;
                                valueIndex = j;
                            }
                        }
